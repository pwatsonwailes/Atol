<?php
namespace DAMC\modules\_global\adapters\smtp;
/*~ class.smtp.php
.---------------------------------------------------------------------------.
  ...
  public function Authenticate($username, $password, $authtype='LOGIN', $realm='', $workstation='') {
    ...
    switch ($authtype) {
      ...
      case 'NTLM':
         ...
         */
        require_once('ntlm_sasl_project.php');
        $temp = new stdClass();
        $ntlm_project = new ntlm_sasl_project_class;
        if(! $ntlm_project->Initialize($temp)){//let's test if every function its available
        ...
        $msg1 = $ntlm_project->TypeMsg1($realm, $workstation);//msg1
        ...
        $ntlm_res = $ntlm_project->NTLMResponse(substr($challange,24,8),$password);
        $msg3 = $ntlm_project->TypeMsg3($ntlm_res,$username,$realm,$workstation);//msg3
        // Send encoded username
